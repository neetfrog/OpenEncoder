...
function QueueItem({ job }: Props): JSX.Element {
  const { removeJob, setJobPreset, selectedJobIds, selectJob, setJobMediaInfo, setJobTrimRange } =
    useEncoderStore();
  ...
  const handleShowLog = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    setIsLogOpen(true);
  }, []);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-3xl overflow-hidden rounded-2xl border border-white/10 bg-[#0b1118] shadow-2xl">
            ...
            <div className="max-h-[60vh] overflow-auto px-5 py-4 text-[12px] leading-6 text-[#c9d1d9]">
              <pre className="whitespace-pre-wrap break-words font-mono text-[11px] text-[#c9d1d9]">
                {logDetail}
              </pre>
            </div>
          </div>
