...
const api = {
  ...
  probe: (filePath: string): Promise<MediaInfo> =>
    ipcRenderer.invoke(IPC.PROBE, filePath),

  // Encoding
  encodeStart: (payload: EncodeStartPayload): Promise<void> =>
    ipcRenderer.invoke(IPC.ENCODE_START, payload),
  encodeCancel: (jobId: string): Promise<void> =>
    ipcRenderer.invoke(IPC.ENCODE_CANCEL, jobId),
  encodeCancelAll: (): Promise<void> =>
    ipcRenderer.invoke(IPC.ENCODE_CANCEL_ALL),
